...
class Lumine_Base extends Lumine_EventListener
{
	...
	public function get( $pk, $pkValue = null )
	{
		...
		if( !empty($pk) && ! empty($pkValue) )
		{
			...
			if( empty($list) )
			{
				Lumine_Log::warning( 'A entidade '.$this->_getName().'  possui chave primï¿½ria. Especifique um campo.');
				return 0;
			}
	 ...
	 * @return string
	 */
	public function toXML( $utf8=true )
	{
	   return Lumine_Util::array2xml( $this->toArray(), $utf8 );
	}
	 ...
	 * @return string 
	 */
	public function toJSON($utf8=false){
		return Lumine_Util::json($this->toArray(), $utf8);
	}
	 ...
	 * @return stdClass
	 */
	public function toObject($format='%s', $className = 'stdClass'){
		// se a opcao de autocast for definida e for stdClass
		if($className == 'stdClass' && $this->_getConfiguration()->getOption('auto_cast_dto') == 1){
	 ...
	 * @return string
	 */
	public function allToXML( $utf8 = true )
	{
		return Lumine_Util::array2xml( $this->allToArray(), $utf8 );
	}
	 ...
	 * @return string
	 */
	public function allToJSON($utf8 = false){
		return Lumine_Util::json($this->allToArray(), $utf8);
	}
	 ...
	 * @return array
	 */
	public function allToObject($format = '%s', $className = 'stdClass'){
		// se a opcao de autocast for definida e for stdClass
		if($className == 'stdClass' && $this->_getConfiguration()->getOption('auto_cast_dto') == 1){
			$className = sprintf($this->_getConfiguration()->getOption('dto_format'), get_class($this));
		}
		...
		while($this->fetch())
		{
			$obj = $this->toObject($format, $className);
			$nova[] = $obj;
		}
	...
	protected function _addForeignRelation($name, $type, $class, $linkOn, $table = null, $column = null, $lazy = false)
	{
      ...
      *
      * @param $entityName Nome da entidade que sera recuperada
      * @author Hugo Ferreira da Silva
      * @link http://www.hufersil.com.br/lumine
	...
	protected function _insertSQL( $opt = null )
	{
		...
		foreach($this->_definition as $name => $def)
		{
			...
			if( !empty($def['options']['autoincrement']) )
			{
				...
				// se nao estiver definida na entidade, tenta pegar a padrï¿½o para todo o banco
	...
	protected function _updateSQL( $whereAddOnly = false )
	{
		...
		foreach($this->_definition as $name => $def)
		{
			...
			//if( isset($valor) ) {
				...
				//compara se tipo de campo é varchar
				if(strtoupper($def['type'])=="VARCHAR")
				{
					if( array_key_exists($name, $this->_original_dataholder) && strcmp($this->_original_dataholder[$name],$valor)==0)
					{
